...
def project_cartesian_image_into_polar_image(image, origin=None):
    """Projects a Cartesian PIL Image into ("image") into a Polar PIL Image.
    "origin" is a tuple of (x0, y0) and defaults to the center of the image."""
    
    image.convert('RGB')
    data = np.array(image)
    polar_grid, r, theta = reproject_image_into_polar(data, origin)
    polar_image = Image.fromarray(polar_grid)
    return polar_image
